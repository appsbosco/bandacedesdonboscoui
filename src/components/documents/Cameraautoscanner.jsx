// FILE: src/components/scanner/CameraAutoScanner.js

import React, { useState, useCallback, useEffect, useRef } from "react";
import PropTypes from "prop-types";
import { useCamera } from "../../hooks/useCamera";
import { useDocumentDetection } from "../../hooks/useDocumentDetection";
import { perspectiveTransform } from "../../utils/documentDetection";
import { captureHighQualityFrame } from "../../utils/imageProcessing";
import { SCANNER_CONFIG, SCANNER_MESSAGES, DOCUMENT_TYPES } from "../../utils/constants";
...
export function CameraAutoScanner({ onCapture, onCancel, documentType = "PASSPORT" }) {
  ...
  const [capturedCorners, setCapturedCorners] = useState(null);

  const cameraStartTimeRef = useRef(0);
  const hasTriggeredCaptureRef = useRef(false);

  const docTypeInfo = DOCUMENT_TYPES[documentType] || DOCUMENT_TYPES.PASSPORT;
  ...
  const handleAutoCapture = useCallback(
    (corners) => {
      ...
      setTimeout(() => {
        try {
          const fullFrame = captureHighQualityFrame(videoRef.current);

          // Aplicar corrección de perspectiva si tenemos esquinas
          let processedCanvas;
          if (corners && corners.length === 4) {
            processedCanvas = perspectiveTransform(fullFrame, corners, 1600, 1100);
          } else {
            processedCanvas = fullFrame;
          }
  ...
  const {
    ...
    onReadyToCapture: handleAutoCapture,
    streakFrames: 10,
  });
  ...
  const handleManualCapture = useCallback(() => {
    ...
    handleAutoCapture(corners);
  }, [phase, videoRef, readyToCapture, corners, handleAutoCapture]);
    ...
    <div className="camera-fullscreen bg-black">
      ...
      <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-start z-20">
        ...
        >
          ...
          {phase === "scanning" &&
            (readyToCapture ? "¡Listo para capturar!" : `${docTypeInfo.label}`)}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 flex flex-col items-center z-20">
          <button
            ...
            className={`w-20 h-20 rounded-full border-4 flex items-center justify-center transition-all duration-300 ${
              readyToCapture
                ? "border-emerald-400 bg-emerald-400/30 scale-110"
                : "border-white/30 bg-white/5 opacity-50 cursor-not-allowed"
            }`}
          >
            <div
              className={`w-14 h-14 rounded-full transition-all duration-300 ${
                readyToCapture ? "bg-emerald-400" : "bg-white/50"
              }`}
            />
          </button>
          ...
          <p className="text-white/50 text-xs mt-3">
            {readyToCapture
              ...
              : "Alinea el documento para habilitar"}
          </p>
        </div>
      )}
