...
export default function EventDrawer({ open, event, isAdmin, onClose, onEdit, onDelete }) {
  ...
  const viewModel = getEventDrawerViewModel(event);
  const panelClasses = getPanelClasses(isMobile, visible);
    ...
    <>
      ...
      <div role="dialog" aria-modal="true" aria-label={event.title} className={panelClasses}>
        ...
        <DrawerHeader
          ...
          onClose={handleClose}
          roseParadeMeta={viewModel.roseParadeMeta}
        />
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-8 sm:px-6">
          ...
          {viewModel.roseParadeMeta?.isParadeDay && (
            <div className="mb-6">
              <RoseParadeEventCountdown event={event} />
            </div>
          )}

          <EventAvatarStack roseParadeMeta={viewModel.roseParadeMeta} />

          <SummaryStrip
            category={viewModel.category.label}
            date={viewModel.shortDate}
            place={event.place}
            roseParadeMeta={viewModel.roseParadeMeta}
            type={event.type}
          />

          {viewModel.showRoseParadeContextPanel && (
            <RoseParadeContextPanel context={viewModel.roseParadeContext} />
          )}

          <EventInfoGrid infoCards={viewModel.infoCards} />
          <EventStatusPills statusPills={viewModel.statusPills} />
          <BusCapacityGrid busCapacities={viewModel.busCapacities} />

          <EventDescriptionSection
            categoryLabel={viewModel.category.label}
            description={event.description}
            roseParadeContext={viewModel.roseParadeContext}
            roseParadeMeta={viewModel.roseParadeMeta}
          />
        ...
        {isAdmin ? (
          ...
          isMobile && <MobileCloseFooter onClose={handleClose} />
        )}
      </div>
    </>,
    document.body
...
function getEventDrawerViewModel(event) {
  ...
  const roseParadeContext = roseParadeMeta ? getRoseParadeContext(event, category.label) : null;
  const scheduleItems = getScheduleItems(event);
  const paymentSummary = getPaymentSummary(hasTransportPayment, transportFee);

  return {
    busCapacities,
    category,
    formattedDate: formatDateEs(event.date),
    formattedTime: event.time ? normalizeTimeTo12h(event.time) : "",
    imgSrc: getEventImage(event.type),
    ...
    roseParadeContext,
    roseParadeMeta,
    shortDate: formatDateEs(event.date, "short"),
    showRoseParadeContextPanel: Boolean(roseParadeContext && !roseParadeMeta.isParadeDay),
    statusPills: getStatusPills(event, priority, roseParadeMeta),
  };
}
...
function getPaymentSummary(hasTransportPayment, transportFee) {
  if (!hasTransportPayment) return "";
  if (transportFee <= 0) return "Pago habilitado";
  return `Pago habilitado · ₡${transportFee.toLocaleString("es-CR")}`;
}
...
function getInfoCards({
  ...
  return [
    ...
    .filter(Boolean)
    .map((card) => ({ ...card, accent: category.accent }));
}

function getStatusPills(event, priority, roseParadeMeta) {
  return [
    event.priority === "high" && {
      key: "priority",
    ...
    event.type && { key: "type", label: event.type },
    roseParadeMeta && { key: "rose-parade", label: roseParadeMeta.label },
  ].filter(Boolean);
}

function getPanelClasses(isMobile, visible) {
  if (isMobile) {
    return `fixed inset-x-0 bottom-0 z-[1300] flex max-h-[94dvh] flex-col overflow-hidden rounded-t-[32px] bg-[#FAFAFB] shadow-[0_-24px_72px_rgba(15,23,42,0.2)] transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${
      visible ? "translate-y-0" : "translate-y-full"
    }`;
  }

  return `fixed inset-y-0 right-0 z-[1300] flex w-full max-w-[560px] flex-col overflow-hidden bg-[#FAFAFB] shadow-[-24px_0_80px_rgba(15,23,42,0.18)] transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${
...
function DrawerHeader({ category, onClose, roseParadeMeta }) {
  return (
    <header className="relative flex h-16 shrink-0 items-center justify-between bg-[#FAFAFB] px-5">
      ...
      <div
        ...
        aria-hidden="true"
      >
        {roseParadeMeta ? "🌹" : category.icon}
      </div>
    </header>
  );
}

function HeroImage({ category, imgSrc, title, type }) {
  return (
    <div className="relative h-56 overflow-hidden rounded-[28px] bg-slate-100 shadow-sm sm:h-64">
      <img src={imgSrc} alt={type ?? title} className="h-full w-full object-cover" />
...
function EventTitleSection({ event, formattedDate, formattedTime, roseParadeMeta }) {
  return (
...
function EventInfoGrid({ infoCards }) {
...
function IconBadge({ accent, children }) {
  return (
...
function EventStatusPills({ statusPills }) {
  if (!statusPills.length) return null;

  return (
    <div className="mt-4 flex flex-wrap gap-2">
      {statusPills.map((pill) => (
        <StatusPill key={pill.key} label={pill.label} tone={pill.tone} />
      ))}
    </div>
  );
}
...
function BusCapacityGrid({ busCapacities }) {
  if (!busCapacities.length) return null;
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
        {busCapacities.map((bus) => (
          ...
          >
            ...
            <p className="mt-1 text-lg font-black text-slate-950">{bus.capacity} cupos</p>
          </div>
        ))}
      </div>
    </section>
...
function EventDescriptionSection({ categoryLabel, description, roseParadeContext, roseParadeMeta }) {
    ...
    <section className="pb-2 pt-8">
      <p
        className={`text-[10px] font-bold uppercase tracking-[0.18em] ${
          content.isRoseFallback ? "text-rose-500" : "text-slate-400"
        }`}
      >
        {content.eyebrow}
      </p>
      ...
      <p className="mt-3 text-sm leading-7 text-slate-600 sm:text-base">{content.description}</p>
    </section>
  );
}

function getDescriptionContent({ categoryLabel, description, roseParadeContext, roseParadeMeta }) {
  if (description) {
    return {
      eyebrow: roseParadeMeta ? "Pasadena 2027" : categoryLabel,
      title: roseParadeMeta ? "Costa Rica marcha hacia Pasadena" : "Detalles del evento",
      description,
      isRoseFallback: false,
    };
  }

  if (!roseParadeContext) return null;

  return {
    eyebrow: "Pasadena 2027",
    title: roseParadeContext.title,
    description: roseParadeContext.description,
    isRoseFallback: true,
  };
}
...
function CreatedAt({ createdAt }) {
  ...
  return (
    <p className="pt-5 text-xs text-slate-400">
      Creado el {new Date(Number(createdAt) || createdAt).toLocaleDateString("es-CR")}
    </p>
  );
}
...
function MobileCloseFooter({ onClose }) {
  return (
...
function SvgIcon({ children, className = "h-3.5 w-3.5", strokeWidth = "2.2" }) {
  return (
    <svg
      className={className}
      fill="none"
...
DrawerHeader.propTypes = { category: PropTypes.object.isRequired, onClose: PropTypes.func.isRequired, roseParadeMeta: PropTypes.object };
HeroImage.propTypes = { category: PropTypes.object.isRequired, imgSrc: PropTypes.string.isRequired, title: PropTypes.string.isRequired, type: PropTypes.string };
EventTitleSection.propTypes = { event: PropTypes.object.isRequired, formattedDate: PropTypes.string.isRequired, formattedTime: PropTypes.string, roseParadeMeta: PropTypes.object };
EventAvatarStack.propTypes = { roseParadeMeta: PropTypes.object };
SummaryStrip.propTypes = { category: PropTypes.string.isRequired, date: PropTypes.string, place: PropTypes.string, roseParadeMeta: PropTypes.object, type: PropTypes.string };
EventInfoGrid.propTypes = { infoCards: PropTypes.array.isRequired };
InfoCard.propTypes = { icon: PropTypes.node.isRequired, label: PropTypes.string.isRequired, value: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired, accent: PropTypes.string, wide: PropTypes.bool };
IconBadge.propTypes = { accent: PropTypes.string, children: PropTypes.node.isRequired };
EventStatusPills.propTypes = { statusPills: PropTypes.array.isRequired };
StatusPill.propTypes = { label: PropTypes.string.isRequired, tone: PropTypes.string };
BusCapacityGrid.propTypes = { busCapacities: PropTypes.array.isRequired };
RoseParadeContextPanel.propTypes = { context: PropTypes.object.isRequired };
EventDescriptionSection.propTypes = { categoryLabel: PropTypes.string.isRequired, description: PropTypes.string, roseParadeContext: PropTypes.object, roseParadeMeta: PropTypes.object };
CreatedAt.propTypes = { createdAt: PropTypes.oneOfType([PropTypes.string, PropTypes.number]) };
AdminFooter.propTypes = { event: PropTypes.object.isRequired, isMobile: PropTypes.bool.isRequired, onClose: PropTypes.func.isRequired, onDelete: PropTypes.func.isRequired, onEdit: PropTypes.func.isRequired };
MobileCloseFooter.propTypes = { onClose: PropTypes.func.isRequired };
SvgIcon.propTypes = { children: PropTypes.node.isRequired, className: PropTypes.string, strokeWidth: PropTypes.string };
