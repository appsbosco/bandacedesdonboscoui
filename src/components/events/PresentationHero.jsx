...
export default function PresentationHero({
  ...
  if (roseParadeMeta?.isParadeDay) {
    return (
      <RoseParadeFeaturedHero
        event={event}
        imgSrc={imgSrc}
        isAdmin={isAdmin}
        days={days}
...
function RoseParadeFeaturedHero({ event, imgSrc, isAdmin, days, onViewDetails, onEdit, onDelete }) {
  return (
    <article className="group relative isolate overflow-hidden rounded-[28px] bg-slate-950 shadow-xl shadow-slate-900/15 ring-1 ring-slate-900/10">
      <img
        src={imgSrc}
        alt={event.type ?? "Desfile de las Rosas 2027"}
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-slate-950/35 via-transparent to-red-950/20" />

      <div className="relative flex min-h-[360px] flex-col justify-between gap-5 p-5 sm:min-h-[390px] sm:p-7">
        <div className="flex items-start justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-white/15 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.12em] text-white shadow-sm backdrop-blur-md">
          ...
          {isAdmin && (
            <div className="flex shrink-0 gap-1.5">
              <FeaturedAdminBtn onClick={() => onEdit(event)} label="Editar evento">
                <EditIcon />
              </FeaturedAdminBtn>
              <FeaturedAdminBtn onClick={() => onDelete(event)} label="Eliminar evento" danger>
                <TrashIcon />
              </FeaturedAdminBtn>
            </div>
          )}
        ...
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-white/70">
            Rumbo a Pasadena · 1 de enero de 2027
          </p>
          <h3 className="mt-2 max-w-3xl text-3xl font-bold leading-tight tracking-tight text-white drop-shadow-sm sm:text-4xl">
            {event.title}
          </h3>
          {event.description && (
        ...
        <RoseParadeEventCountdown event={event} featured />

        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm font-semibold text-white/90">
            <span>{days} días restantes</span>
            <span className="text-white/35">·</span>
            <span>Hasta el {formatDateEs(event.date)}</span>
            {event.time && (
              <>
                <span className="text-white/35">·</span>
                <span>{normalizeTimeTo12h(event.time)}</span>
              </>
            )}
          </div>
          ...
          <button
            type="button"
            onClick={() => onViewDetails(event)}
            className="inline-flex items-center gap-1.5 rounded-xl border border-white/20 bg-white/15 px-4 py-2.5 text-xs font-bold text-white backdrop-blur-md transition-all duration-200 hover:bg-white/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80 active:scale-[0.98]"
          >
            Ver detalles
            <ArrowIcon />
          </button>
...
function FeaturedAdminBtn({ onClick, label, danger = false, children }) {
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={label}
      className={`flex h-9 w-9 items-center justify-center rounded-full border text-white backdrop-blur-md transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80 active:scale-95 ${
        danger
          ? "border-red-300/30 bg-red-500/25 hover:bg-red-500/40"
...
RoseParadeFeaturedHero.propTypes = {
  event: PropTypes.object.isRequired,
