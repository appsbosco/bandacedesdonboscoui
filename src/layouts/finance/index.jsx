  ...
  const handleSubmit = async () => {
    try {
      await openSession({
        variables: {
          businessDate: today,
          openingCash: parseCRC(openingCash) || 0,
...
const CloseSessionModal = ({ session, expectedCashNow, onClose, onSuccess }) => {
  ...
  const diff = countedCash ? counted - (expectedCashNow ?? 0) : null;
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500">Esperado: {formatCRC(expectedCashNow ?? 0)}</p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-400 transition-colors"
...
const MovementItem = ({ item, type, onVoid }) => {
  ...
  const inferredScope = scopeOf(item);
...
const FinanceDashboard = () => {
  ...
  const [movScopeFilter, setMovScopeFilter] = useState("all");
  const [notice, showNotice] = useNotice();
  ...
  const totalSalesAll = activeSales.reduce((a, s) => a + s.total, 0);
  const totalExpensesAll = activeExpenses.reduce((a, e) => a + e.amount, 0);
  const netAll = totalSalesAll - totalExpensesAll;

  const totalSalesSession = activeSales
    .filter((s) => scopeOf(s) === "SESSION")
    .reduce((a, s) => a + s.total, 0);
  const totalExpensesSession = activeExpenses
    .filter((e) => scopeOf(e) === "SESSION")
    .reduce((a, e) => a + e.amount, 0);
  const netSession = totalSalesSession - totalExpensesSession;

  const totalSalesExternal = activeSales
    .filter((s) => scopeOf(s) === "EXTERNAL")
    .reduce((a, s) => a + s.total, 0);
  const totalExpensesExternal = activeExpenses
    .filter((e) => scopeOf(e) === "EXTERNAL")
    .reduce((a, e) => a + e.amount, 0);
  const netExternal = totalSalesExternal - totalExpensesExternal;

  // Efectivo esperado para el cierre (solo CASH SESSION)
  const cashSalesSession = activeSales
    .filter((s) => scopeOf(s) === "SESSION" && s.paymentMethod === "CASH")
    .reduce((a, s) => a + s.total, 0);
  const cashExpensesSession = activeExpenses
    .filter((e) => scopeOf(e) === "SESSION" && e.paymentMethod === "CASH")
    .reduce((a, e) => a + e.amount, 0);
  const expectedCashNow = (session?.openingCash || 0) + cashSalesSession - cashExpensesSession;

  // ── Movimientos combinados + filtros ─────────────────────────────────────
  const allMovements = [
  ...
  const filtered = allMovements.filter((m) => {
    ...
    const s = scopeOf(m);
    if (movScopeFilter === "session" && s !== "SESSION") return false;
    if (movScopeFilter === "external" && s !== "EXTERNAL") return false;
    return true;
  });
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                ...
                {session.status === "OPEN" && (
                  <p className="text-xs text-emerald-900/70 mt-2">
                    Efectivo esperado: <b>{formatCRC(expectedCashNow)}</b>
                  </p>
                )}
              </div>
              ...
              <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4">
                ...
                <p className="text-xs text-indigo-900/70 mt-2">No afectan el arqueo/cierre.</p>
              </div>
            </div>
