...
export default function FlightCard({ flight, onEdit, onDelete, onManagePassengers }) {
  ...
  const itineraryLocked = Boolean(flight.itinerary?.isLocked);

  return (
    <div className={`bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all overflow-hidden ${itineraryLocked ? "border-amber-300 ring-1 ring-amber-100" : "border-gray-200"}`}>
      {/* Top bar */}
      <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-100">
        ...
        <div className="flex items-center gap-1">
          {itineraryLocked && (
            <span className="inline-flex items-center gap-1 rounded-lg bg-amber-100 px-2.5 py-1.5 text-xs font-bold text-amber-800" title={`El itinerario ${flight.itinerary.name} está bloqueado`}>
              🔒 Bloqueado
            </span>
          )}
          <button
            onClick={() => onEdit(flight)}
            disabled={itineraryLocked}
            ...
            title={itineraryLocked ? "El itinerario está bloqueado" : "Editar vuelo"}
          >
          ...
          <button
            ...
            title={itineraryLocked ? "El itinerario está bloqueado" : "Eliminar vuelo"}
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="px-4 pb-4 flex items-center justify-between">
        <button
          ...
          disabled={itineraryLocked}
          className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gray-50 hover:bg-gray-100 border border-gray-100 hover:border-gray-200 transition-all group disabled:cursor-not-allowed disabled:opacity-40"
        >
