...
export default function TourSelfServiceDocuments({ participant, documentSummary, documentSummaryLoading, onSaveInfo, saveLoading, onConfirm, confirmLoading, confirmError, isParentView = false, documentsPath = "/documents" }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="bg-white rounded-2xl border border-gray-200 p-5"><div className="flex justify-between mb-1"><h3 className="text-sm font-bold">Pasaporte</h3>{!isParentView && <Link to={documentsPath} className="text-xs font-semibold text-blue-600">Corregir en Mis Documentos →</Link>}</div><p className="text-xs text-gray-500 mb-4">Revisa que estos datos coincidan exactamente con el pasaporte físico.</p>{documentSummaryLoading ? <div className="h-24 bg-gray-100 rounded-xl animate-pulse" /> : <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4"><DocumentField label="Nombre completo" value={passport?.fullName || [passport?.givenNames, passport?.surname].filter(Boolean).join(" ")} /><DocumentField label="Nombres" value={passport?.givenNames} /><DocumentField label="Apellidos" value={passport?.surname} /><DocumentField label="Número de pasaporte" value={passport?.passportNumber || passport?.documentNumber || participant.passportNumber} /><DocumentField label="Fecha de nacimiento" value={displayDate(passport?.dateOfBirth)} /><DocumentField label="Sexo" value={displaySex(passport?.sex)} /><DocumentField label="Nacionalidad" value={passport?.nationality} /><DocumentField label="País emisor" value={passport?.issuingCountry} /><DocumentField label="Fecha de emisión" value={displayDate(passport?.issueDate)} /><DocumentField label="Vencimiento" value={displayDate(passport?.expirationDate || participant.passportExpiry)} status={participant.passportNumber ? passportStatus : "missing"} /></div>}{passportStatus !== "ok" && <p className="mt-3 text-xs text-amber-700">{isParentView ? "Contacta al administrador para actualizar el pasaporte de tu hijo." : "El pasaporte está incompleto o no vigente. Corrígelo en Mis Documentos."}</p>}</section>
    <section className="bg-white rounded-2xl border border-gray-200 p-5"><div className="flex justify-between mb-1"><h3 className="text-sm font-bold">Visa</h3>{!isParentView && <Link to={documentsPath} className="text-xs font-semibold text-blue-600">Corregir en Mis Documentos →</Link>}</div><p className="text-xs text-gray-500 mb-4">Revisa todos los datos impresos en la visa antes de confirmar.</p>{documentSummaryLoading ? <div className="h-24 bg-gray-100 rounded-xl animate-pulse" /> : <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4"><DocumentField label="Estado" value={participant.hasVisa ? "Registrada" : "No — pendiente"} /><DocumentField label="Tipo de visa" value={visa?.visaType} /><DocumentField label="Número de control" value={visa?.visaControlNumber} /><DocumentField label="País emisor" value={visa?.issuingCountry} /><DocumentField label="Fecha de emisión" value={displayDate(visa?.issueDate)} /><DocumentField label="Vencimiento" value={displayDate(visa?.expirationDate || participant.visaExpiry)} status={visaStatus} /></div>}{(!participant.hasVisa || visaStatus !== "ok") && <p className="mt-3 text-xs text-amber-700">{isParentView ? "Contacta al administrador para actualizar la visa de tu hijo." : "La visa está incompleta o no vigente. Corrígela en Mis Documentos."}</p>}</section>
    <section className="bg-white rounded-2xl border border-gray-200 p-5"><h3 className="text-sm font-bold mb-3">Permiso de salida</h3><DocumentField label="Estado" value={participant.hasExitPermit ? "Registrado" : "No registrado"} /></section>
