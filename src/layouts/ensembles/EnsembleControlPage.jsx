...
function StatePill({ state }) {
  ...
  return (
    <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold border ${CLS[state] || "bg-gray-100 text-gray-500 border-gray-200"}`}>
      {state || "—"}
    </span>
  );
}
    ...
    <tr className="border-b border-gray-50">
      {[0, 1, 2, 3].map((i) => (
        <td key={i} className="px-4 py-3">
          <div className="h-4 bg-gray-100 rounded-full animate-pulse" style={{ width: `${50 + i * 12}%` }} />
        </td>
      ))}
    </tr>
  );
...
function FilterBar({ filter, facets, setSearchText, setFilterField, clearFilters, inputRef }) {
  ...
  return (
    <div className="flex flex-wrap gap-2 items-center">
      ...
      {hasFilters && (
        <button
          onClick={() => { setInputVal(""); clearFilters(); }}
...
function BulkBar({ count, tabType, applying, onAction, onClear }) {
  ...
  return (
    <div className={`px-4 py-2.5 flex items-center gap-3 flex-wrap border-b ${isAdd ? "bg-blue-50 border-blue-100" : "bg-red-50 border-red-100"}`}>
      <span className={`text-xs font-semibold ${isAdd ? "text-blue-700" : "text-red-700"}`}>
        {count} seleccionado{count !== 1 ? "s" : ""}
      </span>
      <button
        onClick={onClear}
        className={`text-xs underline ${isAdd ? "text-blue-500 hover:text-blue-700" : "text-red-400 hover:text-red-600"}`}
      >
        Limpiar
      </button>
      <button
        onClick={onAction}
        disabled={applying}
        className={`ml-auto px-4 py-1.5 rounded-xl text-xs font-bold text-white disabled:opacity-40 transition-all ${isAdd ? "bg-blue-600 hover:bg-blue-700" : "bg-red-600 hover:bg-red-700"}`}
      >
        {applying
          ? "Aplicando…"
          : isAdd
...
function EnsembleTable({ tabType, hook, applying, onBulkAction, onRowClick, searchInputRef }) {
  ...
  const emptyMessage = tabType === "members"
    ? { icon: "👥", title: "Sin miembros", sub: "Esta agrupación aún no tiene miembros asignados." }
    : { icon: "✅", title: "Todos asignados", sub: "Todos los integrantes ya son miembros de esta agrupación." };
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="space-y-0.5">
                            ...
                            {user.instrument && (
                              <p className="text-[11px] text-gray-500 font-medium">{user.instrument}</p>
                            )}
...
export default function EnsembleControlPage() {
  ...
  const { stats: instrStats, loading: instrLoading, refetch: refetchInstrStats } =
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-5 py-4 mb-5">
          <div className="flex flex-wrap items-start gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap flex-shrink-0">
              ...
              >
                Miembros
                <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${activeTab === 0 ? "bg-white/20" : "bg-gray-100 text-gray-500"}`}>
                  {membersTotal}
                </span>
              </button>
              ...
              >
                Disponibles
                <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${activeTab === 1 ? "bg-white/20" : "bg-blue-50 text-blue-600"}`}>
                  {availableTotal}
                </span>
              </button>
